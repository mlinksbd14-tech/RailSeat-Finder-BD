...
🔗 <a href="${finalBookUrl}">🎟️ Book Now on Railway</a>`;
  ...
  // ====================================================
  // 🚉 LIVE STATION PIDS (Platform Display Screen) MODULE
  // ====================================================
  function initStationPidsModule() {
    const stationPidsModal = document.getElementById('stationPidsModal');
    const navStationPidsBtn = document.getElementById('navStationPidsBtn');
    const liveTrackerPidsBtn = document.getElementById('liveTrackerPidsBtn');
    const pidsCloseBtn = document.getElementById('pidsCloseBtn');
    const pidsRefreshBtn = document.getElementById('pidsRefreshBtn');
    const pidsRefreshIcon = document.getElementById('pidsRefreshIcon');
    const pidsFullscreenBtn = document.getElementById('pidsFullscreenBtn');
    const pidsChimeBtn = document.getElementById('pidsChimeBtn');
    const pidsFilterInput = document.getElementById('pidsFilterInput');
    const pidsBoardContainer = document.getElementById('pidsBoardContainer');
    const pidsStationTitle = document.getElementById('pidsStationTitle');
    const pidsStationCodeBadge = document.getElementById('pidsStationCodeBadge');
    const pidsLiveClock = document.getElementById('pidsLiveClock');
    const pidsMovementCountBadge = document.getElementById('pidsMovementCountBadge');
    const pidsCountdownLabel = document.getElementById('pidsCountdownLabel');

    let currentPidsStation = 'Dhaka';
    let currentPidsFilter = 'departures'; // 'departures' | 'arrivals' | 'all'
    let currentPidsData = null;
    let pidsCountdownSeconds = 15;
    let pidsCountdownTimer = null;
    let pidsClockTimer = null;

    const STATION_METADATA = {
      'Dhaka': { nameBn: 'ঢাকা কমলাপুর', nameEn: 'Dhaka Kamalapur', code: 'DK' },
      'Dhaka_Airport': { nameBn: 'ঢাকা বিমানবন্দর', nameEn: 'Dhaka Airport', code: 'DA' },
      'Chattogram': { nameBn: 'চট্টগ্রাম জংশন', nameEn: 'Chattogram Jn', code: 'CG' },
      'Cox\'s_Bazar': { nameBn: 'কক্সবাজার', nameEn: 'Cox\'s Bazar', code: 'CXB' },
      'Sylhet': { nameBn: 'সিলেট জংশন', nameEn: 'Sylhet Jn', code: 'SYL' },
      'Rajshahi': { nameBn: 'রাজশাহী জংশন', nameEn: 'Rajshahi Jn', code: 'RJ' },
      'Khulna': { nameBn: 'খুলনা জংশন', nameEn: 'Khulna Jn', code: 'KLN' },
      'Mymensingh': { nameBn: 'ময়মনসিংহ জংশন', nameEn: 'Mymensingh Jn', code: 'MYM' },
      'Cumilla': { nameBn: 'কুমিল্লা', nameEn: 'Cumilla', code: 'CML' },
      'Feni': { nameBn: 'ফেনী জংশন', nameEn: 'Feni Jn', code: 'FNI' },
      'Santahar': { nameBn: 'সান্তাহার জংশন', nameEn: 'Santahar Jn', code: 'SNT' },
      'Ishwardi': { nameBn: 'ঈশ্বরদী জংশন', nameEn: 'Ishwardi Jn', code: 'ISD' }
    };

    function startPidsClock() {
      if (pidsClockTimer) clearInterval(pidsClockTimer);
      const updateClock = () => {
        if (!pidsLiveClock) return;
        const now = new Date();
        const utc = now.getTime() + (now.getTimezoneOffset() * 60000);
        const bdTime = new Date(utc + (3600000 * 6));
        pidsLiveClock.textContent = bdTime.toLocaleTimeString('en-US', {
          hour12: true,
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit'
      ...
      updateClock();
      pidsClockTimer = setInterval(updateClock, 1000);
    }

    function startPidsCountdown() {
      if (pidsCountdownTimer) clearInterval(pidsCountdownTimer);
      pidsCountdownSeconds = 15;
      if (pidsCountdownLabel) pidsCountdownLabel.textContent = `${pidsCountdownSeconds}s`;

      pidsCountdownTimer = setInterval(() => {
        pidsCountdownSeconds--;
        if (pidsCountdownLabel) pidsCountdownLabel.textContent = `${pidsCountdownSeconds}s`;
        if (pidsCountdownSeconds <= 0) {
          pidsCountdownSeconds = 15;
          fetchStationPids(currentPidsStation, false);
        }
      }, 1000);
    }
    ...
    function playStationChime() {
      try {
        ...
        // Traditional Bangladesh Railway / Airport 3-tone announcement chime: D5 (587Hz) -> F#5 (740Hz) -> A5 (880Hz)
        playTone(587.33, 0, 0.45);
        playTone(739.99, 0.35, 0.55);
        playTone(880.00, 0.70, 0.90);
        showToast('🔔 Railway Station Announcement Chime', 'info');
      } catch (e) {
        console.warn('[PIDS] Chime error:', e);
      }
    }

    function togglePidsFullscreen() {
      const modal = document.getElementById('stationPidsModal');
      const icon = document.getElementById('pidsFullscreenIcon');
      if (!modal) return;
    ...
    async function fetchStationPids(stationName, showLoading = true) {
      if (showLoading && pidsBoardContainer) {
        pidsBoardContainer.innerHTML = `
          <div class="flex flex-col items-center justify-center py-16 text-slate-400 space-y-3">
            ...
            <span class="text-xs font-semibold tracking-wider uppercase">Loading Platform Display for ${escapeHtml(stationName.replace(/_/g, ' '))}...</span>
          </div>
        `;
      }

      if (pidsRefreshIcon) pidsRefreshIcon.classList.add('fa-spin');

      try {
        const res = await fetch(`/api/station-pids?station=${encodeURIComponent(stationName)}`);
        const json = await res.json();

        if (json.success) {
          currentPidsData = json;
          renderPidsBoard();
        } else {
          if (pidsBoardContainer) {
            pidsBoardContainer.innerHTML = `
              <div class="p-8 text-center text-rose-400 space-y-2">
                <i class="fa-solid fa-triangle-exclamation text-2xl"></i>
                <p class="font-bold">Failed to load platform data</p>
                <p class="text-xs text-slate-500">${escapeHtml(json.error || 'Unknown error occurred')}</p>
              </div>
            `;
          }
        }
      } catch (err) {
        console.error('[PIDS Error]:', err);
        if (pidsBoardContainer) {
          pidsBoardContainer.innerHTML = `
            <div class="p-8 text-center text-rose-400 space-y-2">
              <i class="fa-solid fa-network-wired text-2xl"></i>
              <p class="font-bold">Network Connection Error</p>
              <p class="text-xs text-slate-500">Please check server connection</p>
            </div>
          `;
        }
      } finally {
        if (pidsRefreshIcon) pidsRefreshIcon.classList.remove('fa-spin');
      }
    }

    function renderPidsBoard() {
      if (!currentPidsData || !pidsBoardContainer) return;

      const meta = STATION_METADATA[currentPidsStation] || {
        nameBn: currentPidsStation.replace(/_/g, ' '),
        nameEn: currentPidsStation.replace(/_/g, ' '),
        code: (currentPidsStation.substring(0, 3)).toUpperCase()
      };

      if (pidsStationTitle) {
        pidsStationTitle.textContent = `${meta.nameBn} (${meta.nameEn})`;
      }
      if (pidsStationCodeBadge) {
        pidsStationCodeBadge.textContent = meta.code;
      }

      // Filter by movements (departures | arrivals | all)
      let list = [];
      if (currentPidsFilter === 'departures') {
        list = currentPidsData.departures || [];
      } else if (currentPidsFilter === 'arrivals') {
        list = currentPidsData.arrivals || [];
      } else {
        list = currentPidsData.all || [];
      }

      // Filter by search query
      const query = (pidsFilterInput ? pidsFilterInput.value : '').toLowerCase().trim();
      if (query) {
      ...
      if (pidsMovementCountBadge) {
        pidsMovementCountBadge.textContent = `${list.length} Trains Displayed`;
      }

      if (list.length === 0) {
        pidsBoardContainer.innerHTML = `
          <div class="flex flex-col items-center justify-center py-16 text-slate-500 space-y-3 bg-slate-950/40 rounded-2xl border border-slate-800">
            <i class="fa-solid fa-train-subway text-3xl text-slate-600"></i>
            <p class="font-bold text-sm text-slate-300">No scheduled trains matching this filter</p>
            <p class="text-xs text-slate-500">Try changing movement tab (Departures / Arrivals) or clear search filter.</p>
          </div>
        `;
        return;
      }

      // Build High-Visibility Station LED Flipboard Table (Desktop) + Modern Cards (Mobile)
      let html = `
        <!-- Desktop LED Flipboard Table -->
        <div class="hidden md:block overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950 shadow-inner">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-slate-800 bg-slate-900/90 text-[10px] uppercase font-black tracking-wider text-slate-400">
                <th class="py-3 px-4 text-amber-400">Time (সময়)</th>
                <th class="py-3 px-4">Train No & Name</th>
                <th class="py-3 px-4">Route (গন্তব্য / উৎস)</th>
                <th class="py-3 px-4 text-center">Platform</th>
                <th class="py-3 px-4 text-center">Live Status (অবস্থা)</th>
                <th class="py-3 px-4 text-right">Quick Actions</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-850 text-xs font-medium">
      `;

      list.forEach((item, idx) => {
        let statusBadge = '';
        if (item.status === 'BOARDING NOW') {
          statusBadge = `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 font-extrabold text-[11px] border border-amber-500/40 animate-pulse">
          ...
          statusBadge = `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-cyan-500/20 text-cyan-300 font-extrabold text-[11px] border border-cyan-500/40 animate-pulse">
          ...
          statusBadge = `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-300 font-extrabold text-[11px] border border-rose-500/40">
              <i class="fa-solid fa-clock text-[10px]"></i>
              <span>DELAYED +${item.delay_min}m</span>
            </span>
          `;
        } else if (item.status === 'DEPARTED') {
          statusBadge = `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-800 text-slate-400 font-bold text-[10px] border border-slate-700">
              <span>DEPARTED</span>
            </span>
          `;
        } else {
          statusBadge = `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 font-bold text-[11px] border border-emerald-500/40">
        ...
        const rowBg = isEven ? 'bg-slate-950/90' : 'bg-slate-900/40';

        html += `
          <tr class="${rowBg} hover:bg-slate-800/60 transition-colors">
            <!-- Time -->
            <td class="py-3 px-4 whitespace-nowrap">
              <div class="font-mono text-sm sm:text-base font-black text-amber-400 tracking-wider">
                ${escapeHtml(item.display_time)}
              </div>
              <div class="text-[10px] text-slate-500 font-mono">
                ${item.is_origin ? 'Origin Departure' : (item.is_destination ? 'Final Arrival' : 'Stoppage')}
              </div>
            </td>
            ...
            <td class="py-3 px-4">
              <div class="flex items-center space-x-2">
                <span class="px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono font-bold text-[11px] text-cyan-400">
              ...
              <div class="text-[10px] text-slate-400 mt-0.5">
                Off Day: <strong class="text-slate-300">${escapeHtml(item.off_day || 'None')}</strong>
              </div>
            </td>
            ...
            <td class="py-3 px-4">
              ...
              <div class="text-[10px] text-slate-400">
                To: <strong class="text-emerald-400">${escapeHtml(item.to)}</strong>
              </div>
            </td>

            <!-- Platform -->
            <td class="py-3 px-4 text-center whitespace-nowrap">
              <div class="inline-block px-3 py-1 rounded-xl bg-amber-500/10 border border-amber-500/40 text-amber-300 font-black font-mono text-xs shadow-2xs">
            ...
            <td class="py-3 px-4 text-right whitespace-nowrap">
              <div class="inline-flex items-center space-x-1.5">
                <button type="button" class="pids-jump-radar-btn px-2.5 py-1 rounded-lg bg-cyan-600/30 hover:bg-cyan-600 hover:text-white text-cyan-300 border border-cyan-500/40 text-[11px] font-bold transition flex items-center space-x-1 cursor-pointer" data-model="${escapeHtml(item.model)}" title="Track Live on Radar">
                  <i class="fa-solid fa-satellite-dish text-[10px]"></i>
                  <span>Radar</span>
                </button>
                <button type="button" class="pids-jump-seats-btn px-2.5 py-1 rounded-lg bg-emerald-600/30 hover:bg-emerald-600 hover:text-white text-emerald-300 border border-emerald-500/40 text-[11px] font-bold transition flex items-center space-x-1 cursor-pointer" data-model="${escapeHtml(item.model)}" data-from="${escapeHtml(item.from)}" data-to="${escapeHtml(item.to)}" title="Check Seats for this Train">
      ...
      list.forEach(item => {
        let mobileStatusColor = 'emerald';
        if (item.status === 'BOARDING NOW') mobileStatusColor = 'amber';
        if (item.status === 'ARRIVING') mobileStatusColor = 'cyan';
        if (item.status === 'DELAYED') mobileStatusColor = 'rose';
        if (item.status === 'DEPARTED') mobileStatusColor = 'slate';
        ...
        html += `
          <div class="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 space-y-2.5 shadow-sm">
            ...
            <div class="flex items-center justify-between border-b border-slate-800/80 pb-2">
              <div class="flex items-center space-x-2">
                <span class="font-mono text-base font-black text-amber-400">${escapeHtml(item.display_time)}</span>
                <span class="text-[10px] px-1.5 py-0.2 rounded bg-slate-800 text-slate-400 font-mono">
                  ${item.is_origin ? 'Origin' : (item.is_destination ? 'Arrival' : 'Stop')}
                </span>
              </div>
              <div class="px-2.5 py-0.5 rounded-lg bg-amber-500/15 border border-amber-500/40 text-amber-300 font-black font-mono text-xs">
                ${escapeHtml(item.platform)}
              </div>
            </div>

            <!-- Row 2: Train Info & Route -->
            <div>
              <div class="flex items-center space-x-2">
                <span class="px-1.5 py-0.2 rounded bg-slate-800 font-mono font-bold text-[10px] text-cyan-400">#${escapeHtml(item.model)}</span>
                <h4 class="font-bold text-xs text-white">${escapeHtml(item.name)}</h4>
              </div>
              <p class="text-[11px] text-slate-300 font-medium mt-1">${escapeHtml(item.route)}</p>
            </div>

            <!-- Row 3: Status & Action Buttons -->
            <div class="flex items-center justify-between pt-1">
              <div>
                <span class="inline-flex items-center gap-1 text-[11px] font-bold text-${mobileStatusColor}-400">
                  <span class="w-1.5 h-1.5 rounded-full bg-${mobileStatusColor}-400"></span>
                  <span>${escapeHtml(item.status_bn || item.status)}</span>
                </span>
              </div>
              <div class="flex items-center space-x-1.5">
                <button type="button" class="pids-jump-radar-btn px-2.5 py-1 rounded-lg bg-cyan-600/30 hover:bg-cyan-600 text-cyan-300 text-[10px] font-bold border border-cyan-500/30 transition flex items-center space-x-1 cursor-pointer" data-model="${escapeHtml(item.model)}">
                  <i class="fa-solid fa-satellite-dish"></i>
                  <span>Radar</span>
                </button>
                <button type="button" class="pids-jump-seats-btn px-2.5 py-1 rounded-lg bg-emerald-600/30 hover:bg-emerald-600 text-emerald-300 text-[10px] font-bold border border-emerald-500/30 transition flex items-center space-x-1 cursor-pointer" data-model="${escapeHtml(item.model)}" data-from="${escapeHtml(item.from)}" data-to="${escapeHtml(item.to)}">
      ...
      pidsBoardContainer.innerHTML = html;

      // Attach button click delegation for Radar and Seat jumps
      pidsBoardContainer.querySelectorAll('.pids-jump-radar-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          const model = btn.dataset.model;
          closeStationPidsModal();
          // Jump to Live Radar & select train
          if (navLiveTrackerBtn) navLiveTrackerBtn.click();
          setTimeout(() => {
            const trainSelect = document.getElementById('liveTrainSelect');
            if (trainSelect) {
              trainSelect.value = model;
      ...
      pidsBoardContainer.querySelectorAll('.pids-jump-seats-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          ...
          closeStationPidsModal();
          // Jump to Seat Finder
          if (navSeatFinderBtn) navSeatFinderBtn.click();
    ...
    function openStationPidsModal(station = 'Dhaka') {
      currentPidsStation = station;
      if (stationPidsModal) stationPidsModal.classList.remove('hidden');

      // Update station pills UI
      document.querySelectorAll('.pids-station-pill').forEach(pill => {
        if (pill.dataset.station === station) {
          pill.className = 'pids-station-pill px-3 py-1 rounded-lg font-bold transition bg-amber-500 text-slate-950 shadow-xs cursor-pointer shrink-0';
        } else {
          pill.className = 'pids-station-pill px-3 py-1 rounded-lg font-bold transition bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white cursor-pointer shrink-0';
        }
      });

      startPidsClock();
      startPidsCountdown();
      fetchStationPids(station, true);
    }

    function closeStationPidsModal() {
      if (stationPidsModal) stationPidsModal.classList.add('hidden');
      if (pidsCountdownTimer) clearInterval(pidsCountdownTimer);
      if (pidsClockTimer) clearInterval(pidsClockTimer);
      if (document.fullscreenElement) {
    ...
    // Bind Event Handlers
    if (navStationPidsBtn) {
      navStationPidsBtn.addEventListener('click', () => openStationPidsModal(currentPidsStation));
    }

    if (liveTrackerPidsBtn) {
      liveTrackerPidsBtn.addEventListener('click', () => openStationPidsModal(currentPidsStation));
    }

    if (pidsCloseBtn) {
      pidsCloseBtn.addEventListener('click', closeStationPidsModal);
    }

    if (pidsRefreshBtn) {
      pidsRefreshBtn.addEventListener('click', () => fetchStationPids(currentPidsStation, false));
    }

    if (pidsFullscreenBtn) {
      pidsFullscreenBtn.addEventListener('click', togglePidsFullscreen);
    }

    if (pidsChimeBtn) {
      pidsChimeBtn.addEventListener('click', playStationChime);
    }

    if (stationPidsModal) {
      stationPidsModal.addEventListener('click', (e) => {
        if (e.target === stationPidsModal) closeStationPidsModal();
      });
    }

    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && stationPidsModal && !stationPidsModal.classList.contains('hidden')) {
        closeStationPidsModal();
      }
    });
    ...
    document.querySelectorAll('.pids-station-pill').forEach(pill => {
      pill.addEventListener('click', () => {
        const station = pill.dataset.station;
        openStationPidsModal(station);
      });
    });

    // Movement Filter Tabs (Departures / Arrivals / All)
    document.querySelectorAll('.pids-filter-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        currentPidsFilter = tab.dataset.filter;
        document.querySelectorAll('.pids-filter-tab').forEach(t => {
          if (t === tab) {
            t.className = 'pids-filter-tab px-3 py-1 rounded-lg bg-amber-500 text-slate-950 transition flex items-center space-x-1.5 cursor-pointer shadow-xs';
          } else {
            t.className = 'pids-filter-tab px-3 py-1 rounded-lg text-slate-400 hover:text-white transition flex items-center space-x-1.5 cursor-pointer';
          }
        });
        renderPidsBoard();
      });
    ...
    // Search filter input
    if (pidsFilterInput) {
      pidsFilterInput.addEventListener('input', () => {
        renderPidsBoard();
      });
    }
  }
