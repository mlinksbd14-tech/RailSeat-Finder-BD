...
// Load raw BD railway ways (10,215 physical ways, 91,575 coords)
let nodes = [];
let adj = [];
const grid = new Map();
let allWaySegments = []; // for orthogonal projection snapping
...
function initRailGraph() {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        ...
        if (cell) {
          for (const idx of cell) {
            const pt = nodes[idx];
            if (Math.hypot(pt[0] - lat, pt[1] - lon) < 0.00012) {
              return idx;
            }
          }
        }
  ...
  // Find largest component (main trunk network)
  const compSizes = new Map();
  for (let i = 0; i < nodes.length; i++) {
    compSizes.set(comp[i], (compSizes.get(comp[i]) || 0) + 1);
  }
  let mainComp = 0;
  let maxCompSize = 0;
  for (const [c, sz] of compSizes.entries()) {
    if (sz > maxCompSize) {
      maxCompSize = sz;
      mainComp = c;
    }
  ...
  // Connect EVERY non-main component to the nearest node in the main component
  for (let c = 0; c < compCount; c++) {
    if (c === mainComp) continue;
    const cNodes = [];
    for (let i = 0; i < nodes.length; i++) {
      if (comp[i] === c) cNodes.push(i);
    }
    ...
    for (const u of cNodes) {
      const pt1 = nodes[u];
      const gx = Math.floor(pt1[0] * 1000);
      const gy = Math.floor(pt1[1] * 1000);
      for (let r = 0; r <= 35; r++) {
        ...
        if (bestU !== -1 && minD < (r * 0.001)) break;
      }
    }

    if (bestU !== -1 && bestV !== -1) {
      adj[bestU].push({ to: bestV, dist: minD });
      adj[bestV].push({ to: bestU, dist: minD });
      for (const u of cNodes) {
        comp[u] = mainComp;
      }
    }
  ...
  console.log(`[RailRouter] Initialized 100% unified connected rail graph with ${nodes.length} nodes and ${allWaySegments.length} segments`);
}

initRailGraph();

// Find nearest node in spatial grid
...
// Snap a GPS coordinate to the nearest point on physical railway track line segment
function snapToRailTrack(lat, lon) {
  ...
  for (let dx = -5; dx <= 5; dx++) {
    for (let dy = -5; dy <= 5; dy++) {
      const cell = grid.get(`${gx + dx},${gy + dy}`);
      if (cell) {
        candidateIndices.push(...cell);
      }
    }
  }

  for (const u of candidateIndices) {
    const p1 = nodes[u];
    for (const e of adj[u]) {
      const p2 = nodes[e.to];
      const dx = p2[0] - p1[0];
      ...
      if (lenSq === 0) {
        ...
        const uVal = Math.max(0, Math.min(1, ((lat - p1[0]) * dx + (lon - p1[1]) * dy) / lenSq));
        proj = [p1[0] + uVal * dx, p1[1] + uVal * dy];
      }
      const d = Math.hypot(proj[0] - lat, proj[1] - lon);
      if (d < minD) {
        minD = d;
        bestPt = proj;
      }
  ...
  return minD < 0.15 ? [Math.round(bestPt[0] * 100000) / 100000, Math.round(bestPt[1] * 100000) / 100000] : [lat, lon];
}
...
function rdpSimplify(points, epsilon = 0.00003) {
...
function solveTrackBetweenCoords(startCoord, endCoord) {
  ...
  let iterations = 0;
  while (heap.size() > 0 && iterations < 125000) {
    iterations++;
    ...
    if (current === endIdx) {
      ...
      const simplified = rdpSimplify(path, 0.000005);
      const res = simplified.map(p => [Math.round(p[0] * 1000000) / 1000000, Math.round(p[1] * 1000000) / 1000000]);
